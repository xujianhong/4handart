...
class MineFragment : BaseFragment<MinePresenter>(), MineContract.View {
    ...
    override fun requestCancelAccount() {
    ...
    override fun requestAboutUsSuccess(data: AboutUsBean?,type:Int) {
        data?.let {
            if(type==1) {
                startActivity<CommonWebActivity>(Constant.URL_EXTRA to it.aboutUs,Constant.TITLE_EXTRA to "关于我们")
            }
            if(type==2){
                startActivity<CommonWebActivity>(Constant.URL_EXTRA to it.serviceAgent, Constant.TITLE_EXTRA to "投诉/举报")
            }
        }
    }
            ...
            AlertDialog.Builder(activity!!).setTitle("注意")
                ...
                .setPositiveButton("确定", object : DialogInterface.OnClickListener{
                    override fun onClick(dialog: DialogInterface?, which: Int) {
                        ...
                        LitePal.deleteAll(VideoList::class.java) //清空数据表

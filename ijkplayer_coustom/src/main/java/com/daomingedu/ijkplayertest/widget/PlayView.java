            ...
            switch (msg.what) {
                ...
                case CURRONTTIME:
                    ...
                    if (currentPosition <= duration) {
                        //更新时间显示
                        String currentStr =sec2time(currentPosition);
                        if(onBtnListener!=null){
//                            onBtnListener.onTime(currentStr);
                        }
    ...
    private SeekBar.OnSeekBarChangeListener seekBarChangeListener = new SeekBar.OnSeekBarChangeListener() {
        ...
        public void onStartTrackingTouch(SeekBar seekBar) {
            if (videoView.isPlaying()) {
                ...
                if(onBtnListener!=null){
                    onBtnListener.onBtn(OnBtnListener.TYPE_PASUE);
                }
...
//            pasue.setImageResource(R.mipmap.play);
     ...
     * @return
     */
    public PlayView setPath(final String videoPath, final boolean isplay) {
        this.isplay = isplay;
        path = videoPath;

        if (path.contains("http://") &&
                !isGoOn &&
                NetUtils.getConnectedType(mContext.getApplicationContext()) != ConnectivityManager.TYPE_WIFI) {
            ...
            if (isplay) {
                ...
                pasue.setImageResource(R.mipmap.play);
                if(onBtnListener!=null){
                    onBtnListener.onBtn(OnBtnListener.TYPE_PLAY);
                }
...
//                            videoView.pause();
//                        }
                        if (videoView.getCurrentState() != IjkVideoView.STATE_PREPARED) {
                            mHandler.sendEmptyMessage(SHOWPB);
        ...
        videoView.setOnCompletionListener(new IMediaPlayer.OnCompletionListener() {
            ...
            public void onCompletion(IMediaPlayer iMediaPlayer) {
                ...
                pasue.setImageResource(R.mipmap.pasue);
                if(onBtnListener!=null){
                    onBtnListener.onBtn(OnBtnListener.TYPE_COMPLETE);
                }
                mHandler.removeMessages(CURRONTTIME);
...
//        int secondaryProgress = (int) (videoView.getDuration() * i / 100f);
    ...
    public void onConfigurationChanged(Configuration newConfig) {
        ...
        if (mCurrentOrientation == Configuration.ORIENTATION_PORTRAIT) {//竖屏的时候
            ...
            rl_controll.setSystemUiVisibility(View.SYSTEM_UI_FLAG_VISIBLE);//底部虚拟按键
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.KITKAT) {
                rl_controll.setSystemUiVisibility(View.SYSTEM_UI_FLAG_HIDE_NAVIGATION | View.SYSTEM_UI_FLAG_IMMERSIVE_STICKY);
            }
        }
    }
